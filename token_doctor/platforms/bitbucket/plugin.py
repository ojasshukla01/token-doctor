"""Bitbucket plugin: token check via /2.0/user, changelog from blog feed."""
...
DECLARED_ENDPOINTS = ["https://api.bitbucket.org/2.0/user"]
CHANGELOG_FEEDS = [
    "https://blog.bitbucket.org/feed/",
]
DOCS_LINKS = ["https://developer.atlassian.com/cloud/bitbucket/rest/", "https://blog.bitbucket.org/"]

metadata = {
    "platform": "bitbucket",
    "auth_types": ["oauth", "app_password"],
    "documentation_links": DOCS_LINKS,
    "declared_endpoints": DECLARED_ENDPOINTS,
    "sources_monitored": CHANGELOG_FEEDS,
    "data_collected": ["user uuid, username from /2.0/user", "blog feed entries"],
}


def token_checks(token: str, config: Any) -> list[CheckResult]:
    """Validate token via Bitbucket /2.0/user (Bearer or Basic)."""
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            username = data.get("username", data.get("display_name", "?"))
            results.append(
                CheckResult(
                    name="validity",
                    status=CheckStatus.OK,
                    message=f"Token valid (user: {username})",
                    details={"username": username, "uuid": data.get("uuid")},
...
def collect_changes(since: Any) -> list[NormalizedEvent]:
    ...
    for url in CHANGELOG_FEEDS:
        events.extend(safe_fetch_feed(url, "bitbucket", since=since))
    return events
