"""GitLab plugin: token check via /user, changelog from blog/feeds."""
...
DECLARED_ENDPOINTS = ["https://gitlab.com/api/v4/user"]
CHANGELOG_FEEDS = [
    "https://about.gitlab.com/feed.xml",
    "https://about.gitlab.com/blog/feed.xml",
]
DOCS_LINKS = ["https://docs.gitlab.com/ee/api/", "https://about.gitlab.com/blog/"]

metadata = {
    "platform": "gitlab",
    "auth_types": ["oauth", "personal_access_token", "job_token"],
    "documentation_links": DOCS_LINKS,
    "declared_endpoints": DECLARED_ENDPOINTS,
    "sources_monitored": CHANGELOG_FEEDS,
    "data_collected": ["user id, username from /user", "blog/changelog from RSS"],
}


def token_checks(token: str, config: Any) -> list[CheckResult]:
    """Validate token via GitLab /api/v4/user."""
...
def collect_changes(since: Any) -> list[NormalizedEvent]:
    ...
    for url in CHANGELOG_FEEDS:
        events.extend(safe_fetch_feed(url, "gitlab", since=since))
    return events
