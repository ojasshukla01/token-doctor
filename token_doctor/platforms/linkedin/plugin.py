"""
LinkedIn plugin: token check via /v2/me (Marketing API OAuth).

Declared endpoints and developer/changelog feeds.
"""
...
CHANGELOG_FEEDS = [
    "https://www.linkedin.com/feed/rss/articles?topic=developer",
]
DOCS_LINKS = [
...
metadata = {
    ...
    "data_collected": ["me (localizedFirstName, id) for validation", "developer feed"],
}


def token_checks(token: str, config: Any) -> list[CheckResult]:
    """Validate token via LinkedIn /v2/me (Bearer)."""
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            name = data.get("localizedFirstName", data.get("id", "?"))
            results.append(
                CheckResult(
                    name="validity",
...
def collect_changes(since: Any) -> list[NormalizedEvent]:
    ...
    for url in CHANGELOG_FEEDS:
        events.extend(safe_fetch_feed(url, "linkedin", since=since))
    return events
