"""Cloudflare plugin: token check via /user (API token), changelog from blog."""
...
DECLARED_ENDPOINTS = ["https://api.cloudflare.com/client/v4/user"]
CHANGELOG_FEEDS = [
    "https://blog.cloudflare.com/rss/",
]
DOCS_LINKS = ["https://developers.cloudflare.com/api/", "https://blog.cloudflare.com"]

metadata = {
    "platform": "cloudflare",
    "auth_types": ["api_token", "api_key"],
...
def token_checks(token: str, config: Any) -> list[CheckResult]:
    """Validate API token via Cloudflare /client/v4/user (Bearer)."""
...
def collect_changes(since: Any) -> list[NormalizedEvent]:
    ...
    for url in CHANGELOG_FEEDS:
        events.extend(safe_fetch_feed(url, "cloudflare", since=since))
    return events
