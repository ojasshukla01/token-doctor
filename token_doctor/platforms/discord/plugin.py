"""Discord plugin: token check via /users/@me, changelog from developer blog."""
...
DECLARED_ENDPOINTS = ["https://discord.com/api/v10/users/@me"]
CHANGELOG_FEEDS = [
    "https://discord.com/blog/feed",
    "https://discord.com/developers/docs/feed",  # if exists
]
DOCS_LINKS = ["https://discord.com/developers/docs/", "https://discord.com/blog"]

metadata = {
    "platform": "discord",
    "auth_types": ["oauth2", "bot_token"],
    "documentation_links": DOCS_LINKS,
    "declared_endpoints": DECLARED_ENDPOINTS,
    "sources_monitored": CHANGELOG_FEEDS,
    "data_collected": ["user id, username from /users/@me", "blog feed entries"],
}


def token_checks(token: str, config: Any) -> list[CheckResult]:
    """Validate token via Discord /users/@me."""
...
def collect_changes(since: Any) -> list[NormalizedEvent]:
    ...
    for url in CHANGELOG_FEEDS:
        events.extend(safe_fetch_feed(url, "discord", since=since))
    return events
