"""
Instagram plugin: token check via Instagram Graph API /me.

Uses Meta Graph API (graph.instagram.com); same OAuth family as Meta Marketing.
"""
...
metadata = {
    ...
    "data_collected": ["user id, username from /me", "Meta developer blog feed"],
}


def token_checks(token: str, config: Any) -> list[CheckResult]:
    """Validate token via Instagram Graph /me (Bearer)."""
    results: list[CheckResult] = []
    endpoint = "https://graph.instagram.com/v18.0/me"
    try:
...
def collect_changes(since: Any) -> list[NormalizedEvent]:
    ...
    for url in CHANGELOG_FEEDS:
        events.extend(safe_fetch_feed(url, "instagram", since=since))
    return events
